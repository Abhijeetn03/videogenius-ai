  ...
  const handleGenerate = async (e: React.FormEvent<HTMLFormElement>)=>{
    ...
    if(!productImage || !modelImage || !name || !productName || !aspectRatio) return toast('Please fill all the required fields')
      ...
      try {
        ...
        toast.error("Veo API limit reached. Not enough credits to generate video.");
      }
  }
  ...
  return (
    <div className="min-h-screen text-white p-6 md:p-12 mt-28">
      ...
      <form onSubmit={handleGenerate} className="max-w-4xl mx-auto mb-40"> 
        ...
        <Title heading='Create In-Context Image' description="Upload your model and product images to generate stunning AI videos, short-form content and social media posts"/>
        ...
        <div className="flex justify-center items-center gap-4 mt-10 max-sm:flex-col">
          <PrimaryButton disabled={isGenerating} className="px-10 py-3 rounded-md disabled:opacity-70 disabled:cursor-not-allowed max-sm:w-full">
            {isGenerating ? (
              <>
          ...
          <GhostButton type="button" onClick={() => navigate('/result/demo')} className="px-10 py-3 rounded-md max-sm:w-full border border-white/20">
            View Demo Video
          </GhostButton>
        </div>
      </form>
