...
export const createProject = async (req:Request, res: Response) => {
    ...
    console.log('User found:', user?.id, 'Credits:', user?.credits);

    // Auto-create user if they don't exist (for development/testing)
    if(!user) {
        const authSession = (req as any).auth?.session;
        console.log('Creating new user with 10000 credits');
        user = await prisma.user.create({
            data: {
                id: userId,
                email: authSession?.user?.email_addresses?.[0] || 'user@example.com',
                name: authSession?.user?.first_name || 'User',
                image: authSession?.user?.image_url || '',
                credits: 10000
            }
        });
    }

    console.log('Proceeding with project creation, user credits:', user?.credits);
